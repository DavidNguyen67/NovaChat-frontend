...
import { mockFriendRequests } from './config';
...
const FriendRequestList = () => {
  ...
  const { friendRequestsList } = useFriends();
  ...
  const requestData = async () => {
    try {
      ...
      // const data = await friendRequestsList.trigger({
  ...
  const renderContent = () => {
    ...
    if (!dataView.length) {
      if (friendRequestsList.isMutating) {
        return (
        ...
        if (friendRequestsList.error)
          return (
            <FallBack
              error={friendRequestsList.error}
              type="error"
              onRetry={refreshData}
            />
          );
    ...
    return (
      <VirtuosoGrid<Friend, FriendItemProps>
        ...
        data={dataView}
        itemContent={(_, data) => <FriendItem isRequest data={data} />}
  ...
  return (
    <div className="flex flex-col gap-6 p-6 w-full max-w-6xl mx-auto h-full min-h-[60vh]">
          ...
          <CardBody>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold text-foreground flex items-center gap-2">
                {friendRequestsList.isMutating ? (
                  <Skeleton className="w-35 rounded-lg">
